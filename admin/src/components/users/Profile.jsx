...
const Profile = () => {
  ...
  const [profileColor, setProfileColor] = useState('#6366f1') // Default indigo color

  // This would typically come from your backend or state management
  const user = {
    "_id": "675c966c4cf6e3eb4585ad76",
    "first_name": "Tony",
    "last_name": "Stark",
    ...
    "isAdmin": true,
    "isSupreme": false,
    "last_login": new Date("2024-12-24T12:13:00.465Z"),
    "createdAt": new Date("2024-12-13T12:11:08.598Z"),
  }
    ...
    <div className='min-h-screen w-full flex flex-col items-center justify-center relative overflow-hidden bg-gray-900 p-4'>
      <div className='w-full max-w-4xl mb-4 flex justify-end space-x-4'>
        <div className="flex items-center">
          <label htmlFor="glassToggle" className="mr-2 text-white">Glass Effect</label>
        ...
        <div className="flex items-center">
          <label htmlFor="colorPicker" className="mr-2 text-white">Profile Color</label>
          <input
            type="color"
            id="colorPicker"
            value={profileColor}
      ...
      <div className={`w-full max-w-4xl p-8 bg-gray-800 rounded-xl shadow-lg ${glassEffect ? 'bg-opacity-20 backdrop-filter backdrop-blur-lg' : ''}`}>
        <div className='flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6'>
          <div className='flex justify-center items-center z-1 h-40 w-40 rounded-full bg-gray-700 overflow-hidden border-4' style={{ borderColor: profileColor }}>
            <img 
          ...
          <div className='flex-1'>
            ...
            <p className='text-lg text-gray-300 mt-2'>
              Status: <span className='font-semibold text-green-400'>{user.status}</span>
            </p>
          ...
          <div className={`p-4 rounded-lg ${glassEffect ? 'bg-gray-700 bg-opacity-50' : 'bg-gray-700'}`}>
            ...
            <div className='space-y-2'>
              <p className='text-gray-300'>
                ...
                User ID: <span className='font-mono' style={{ color: profileColor }}>{user._id}</span>
              </p>
              <p className='text-gray-300'>
          ...
          <div className={`p-4 rounded-lg ${glassEffect ? 'bg-gray-700 bg-opacity-50' : 'bg-gray-700'}`}>
            ...
            <div className='space-y-2'>
              <p className='text-gray-300'>
                ...
                Last Login: <span style={{ color: profileColor }}>{user.last_login.toLocaleString()}</span>
              </p>
              <p className='text-gray-300'>
                <Calendar className='inline w-5 h-5 mr-2' style={{ color: profileColor }} />
                Account Created: <span style={{ color: profileColor }}>{user.createdAt.toLocaleDateString()}</span>
              </p>
            </div>
          </div>
        </div>
